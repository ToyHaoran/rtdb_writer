...
func main() {
	...
	flag.StringVar(&sql, "sql", "count timeseries", "--sql=xxx")
	flag.BoolVar(&isQuery, "is_query", true, "--isQuery=true")
	flag.Parse()
	...
	if err == nil {
		if isQuery {
			// select * from root.xxx 自动打印结果
			var timeout int64 = 2000
			sessionDataSet, _ := session.ExecuteQueryStatement(sql, &timeout)
